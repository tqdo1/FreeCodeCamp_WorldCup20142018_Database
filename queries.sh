...
echo "$($PSQL "SELECT (SUM(winner_goals) + SUM(opponent_goals))::numeric/COUNT(*) AS avg_goals_all_teams FROM games")"
...
echo "$($PSQL "SELECT DISTINCT year, name FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE round = 'Final' ORDER BY year, name")"
